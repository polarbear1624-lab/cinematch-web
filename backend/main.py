...
nlp_recommender.train_model() # <-- Added this!
...
@app.get("/")
def read_root():
    return {"message": "Welcome to the Mechanical Cinephile API! 🎬"}
...
# --- PHASE 3, 4 & 5: INDIVIDUAL ML ENGINES ---
...
# --- PHASE 6: THE HYBRID MASTER ENGINE ---

# --- PHASE 5.5: THE DISCOVERY ENGINE (BROWSE MODE) ---
@app.get("/browse/")
def browse_movies(genre: str = "All Genres", decade: str = "Any Time", limit: int = 10):
    # We can just borrow the loaded dataframe from the NLP engine since it's already in memory!
    ...
    # If the user asks for a very strict filter and nothing exists
    if df.empty:
        return {"movies": []}
        
    # Grab the top results (You could sort this by popularity if you had a popularity column!)
...
def recommend_hybrid(movie_title: str, genre: str = "All Genres", decade: str = "Any Time"):
    # 1. Ask the Hive Mind (Collaborative) for top 10 taste matches
    hive_mind_titles = collab_recommender.get_recommendations(movie_title, top_n=10)
    
    # 2. Ask the Soul (NLP) for top 10 vibe matches - NOW WITH FILTERS!
